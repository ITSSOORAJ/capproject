...
const EditTurfForm = ({ turf, onSave, onCancel, turfId }) => {
  ...
  const { register, handleSubmit, control, getValues, formState: { errors } } = useForm({
    ...
    defaultValues: {
      ...turf,
      openTime: isValid(new Date(turf.openTime)) ? parse(turf.openTime, 'hh:mm a', new Date()) : null,
      closeTime: isValid(new Date(turf.closeTime)) ? parse(turf.closeTime, 'hh:mm a', new Date()) : null,
    },
  });
  ...
  const onSubmit = (data) => {
    onSave(
      {
        ...data,
        openTime: isValid(data.openTime) ? format(data.openTime, 'hh:mm a') : null,
        closeTime: isValid(data.closeTime) ? format(data.closeTime, 'hh:mm a') : null,
      },
      turfId
    );
  };
